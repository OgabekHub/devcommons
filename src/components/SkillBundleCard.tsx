...
export default function SkillBundleCard({
  ...
  return (
    <div className="flex flex-col rounded-3xl border border-line bg-gradient-to-br from-[#0B0D14] via-[#0F1420] to-[#121929] p-6 shadow-2xl transition-all duration-300 hover:border-cyan-500/30 md:p-8">
      {/* Top Details */}
      <div className="flex flex-col gap-4 border-b border-line pb-6 md:flex-row md:items-start md:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-lg bg-ink/5 px-2.5 py-1 text-xs font-medium text-zinc-400">
              {category}
            </span>
          </div>

          <h3 className="mt-3 text-xl font-extrabold text-fg sm:text-2xl">{title}</h3>
          <p className="mt-1 max-w-2xl text-xs sm:text-sm text-zinc-300 leading-relaxed">{description}</p>
          <div className="mt-3 flex items-center gap-2 text-xs text-zinc-400">
            <span>Curated by <strong className="text-fg font-medium">{author}</strong></span>
            <span>•</span>
        ...
        <div className="flex flex-wrap items-center gap-3 shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-2xl border border-line bg-ink/5 px-4 py-3 text-xs font-semibold text-zinc-300 transition hover:bg-ink/10 hover:text-fg"
      ...
      <div className="mt-6 flex flex-col gap-4 lg:flex-row">
        {/* File Sidebar */}
        <div className="flex flex-row overflow-x-auto gap-2 border-b border-line pb-3 lg:flex-col lg:w-64 lg:overflow-x-visible lg:border-b-0 lg:border-r lg:pb-0 lg:pr-4 shrink-0">
          <div className="hidden lg:flex items-center gap-2 text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">
            ...
            <button
              ...
              className={`flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-left transition-all shrink-0 ${
                selectedItemIdx === idx
                  ...
                  : "text-zinc-400 hover:bg-ink/5 hover:text-zinc-200 border border-transparent"
              }`}
            >
        ...
        {/* Content Viewer */}
        <div className="flex flex-1 flex-col rounded-2xl border border-line bg-[#06080D] p-4 shadow-inner min-w-0">
          <div className="flex items-center justify-between border-b border-line pb-2.5 mb-3">
            <div className="flex items-center gap-2 text-xs font-bold text-zinc-200">
              ...
              <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded bg-ink/10 text-zinc-400">
                {selectedItem?.type}
              </span>
            </div>
            <button
              onClick={handleCopySingle}
              className="flex items-center gap-1.5 rounded-lg border border-line bg-ink/5 px-2.5 py-1 text-xs text-zinc-300 hover:text-fg hover:bg-ink/10 transition"
            >
          ...
          <pre className="overflow-x-auto font-mono text-xs sm:text-sm text-zinc-300 leading-relaxed p-2">
