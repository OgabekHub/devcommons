...
export default async function CollectionPage({ params: { id, locale } }: Props) {
  ...
  return (
    <div className="container mx-auto max-w-6xl px-4 py-8 pb-20">
      <Link
        href={`/profile`}
        className="mb-8 flex w-fit items-center gap-2 rounded-xl border border-line bg-surface-subtle px-4 py-2 text-sm font-medium text-zinc-400 transition-colors hover:border-line-strong hover:bg-ink/5 hover:text-fg"
      >
      ...
      {/* Header */}
      <div className="mb-10 rounded-2xl border border-line bg-[#0F0A1F]/50 p-6 sm:p-8">
          ...
          <div className="flex gap-4">
            ...
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-2xl font-bold text-fg sm:text-3xl">{collection.title}</h1>
                {collection.is_public ? (
                  ...
                  <span className="flex items-center gap-1 rounded-full bg-zinc-500/10 px-2 py-0.5 text-xs font-medium text-zinc-400">
      ...
      {items && items.length > 0 ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="overflow-hidden">
                      <Link
                        ...
                        className="truncate text-lg font-bold text-fg hover:text-brand"
                      >
                        {content.title}
                      </Link>
                ...
                <div className="mt-auto flex items-center justify-between border-t border-line pt-4">
                  <div className="flex gap-2">
                    {isSnippet && content.language && (
                      <span className="rounded-lg bg-ink/5 px-2.5 py-1 text-xs font-medium text-zinc-300">
                        {content.language}
                      </span>
                    )}
                    {content.model && (
        ...
        <div className="rounded-2xl border border-dashed border-line p-12 text-center text-zinc-500">
